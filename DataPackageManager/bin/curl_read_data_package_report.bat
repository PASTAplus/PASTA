...
curl -i -b auth-token=%AUTH_TOKEN_UCARROLL% -G %SERVICE_HOST%/package/report/eml/%SCOPE%/%IDENTIFIER%/%REVISION%
curl -i -b auth-token=%AUTH_TOKEN_UCARROLL% -H "Accept: text/html" -G %SERVICE_HOST%/package/report/eml/%SCOPE%/%IDENTIFIER%/%REVISION%
