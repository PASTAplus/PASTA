...
curl -i -b auth-token=%AUTH_TOKEN_UCARROLL% -X GET "%SERVICE_HOST%/package/search/eml?defType=edismax&q=singledate:*&q=begindate:*&fq=-scope:ecotrends&fq=-scope:lter-landsat*&fl=id,pubdate,begindate,enddate,singledate&sort=score,desc&sort=packageid,asc&debug=false&start=0&rows=10"
