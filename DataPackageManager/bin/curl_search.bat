...
SET SERVICE_HOST=http://package-d.lternet.edu:8080
...
curl -i -b auth-token=%AUTH_TOKEN_UCARROLL% -X GET "%SERVICE_HOST%/package/search/eml?defType=edismax&q=lter&fq=-scope:ecotrends&fq=-scope:lter-landsat*&fl=id,packageid,title,author,organization,pubdate&sort=score,desc&sort=packageid,asc&debug=false&start=0&rows=10"
