...
CREATE TABLE datapackagemanager.resource_registry (
  resource_id VARCHAR(600) NOT NULL,     -- resource id, the primary key
  md5_id VARCHAR(64),                    -- opaque id used for DOI
  ...
  entity_id VARCHAR(500),                -- the entity id (as appears in the URL)
  entity_name VARCHAR(500),              -- the entity name (as appears in the EML)
...
CREATE TABLE datapackagemanager.access_matrix (
  ...
  resource_id VARCHAR(600) NOT NULL,                             -- resource id, a foreign key
