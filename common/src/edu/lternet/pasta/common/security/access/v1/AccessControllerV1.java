 ...
 * into a JAXB {@link AccessType} object, 2) validated using an
 * {@link AccessTypeValidator}, 3) used to construct an {@link AccessMatrix},
 * and 4) evaluated using an {@link AccessMatrixEvaluator}.
 *
 */
public final class AccessControllerV1 extends AbstractAuthTokenAccessController {
    ...
    private final AccessTypeValidator validator;
    private final AccessMatrixEvaluator evaluator;

    public AccessControllerV1() {
        super();
     ...
     */
    public AccessElementValidator getAcrValidator() {
        return new AccessElementValidator(false);
    }
     ...
     *
     * @param access2_1_0
     *            a string that is parsable as an EML 2.1.0 {@code <access>}
     *            element.
     * @return an access matrix that corresponds to the provided {@code
     *         <access>} element string.
     *
     * @see EmlUtility#getAccessType2_1_0(String)
     */
    public AccessMatrix makeAccessMatrix(String access2_1_0) {

        AccessType access = EmlUtility.getAccessType2_1_0(access2_1_0);
    ...
    private RuleType getRuleType(JAXBElement<AccessRule> jaxb) {
        ...
        if (elementName.equalsIgnoreCase(AccessTypeValidator.ALLOW)) {
            return RuleType.ALLOW;
        }
        else if (elementName.equalsIgnoreCase(AccessTypeValidator.DENY)) {
    ...
    private AccessMatrix makeEmptyAccessMatrix(String orderString) {
        ...
        if (orderString.equals(AccessTypeValidator.ALLOW_FIRST)) {
            return new AccessMatrix(Order.ALLOW_FIRST);
        }
        else if (orderString.equals(AccessTypeValidator.DENY_FIRST)) {
     ...
     * @param accessElement
     *            a string containing the EML 2.1.0 {@code <access>} element.
     *
