...
The *reverse proxy* function of the Gatekeeper is implemented using the
``org.eclipse.jetty.proxy.ProxyServlet`` class and specifically looks for
...
ensure the string is HTTP transport layer friendly, and then added to the HTTP
request as part of the *Cookie* header with the name ``auth-token``.

Authentication tokens (specifically, external authentication tokens) are
returned to the client only in responses that originate from an HTTP request
containing LDAP credentials and may be reused by the client in-lieu of the
LDAP credentials for future authentication purposes. In this case, the
...
part of the authentication token is private, the digitally signature prevents
malicious altering of the token, or its individual parts.

When using the external authentication token in a client HTTP request, the
Gatekeeper parses out the authentication token from the *Cookie* header,
